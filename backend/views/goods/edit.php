...
<div class="tab-div">
    ...
    <div id="tabbody-div">
        <form enctype="multipart/form-data" action="<?= \yii\helpers\Url::to(['goods/update'])?>" method="post">
            <table width="90%" id="general-table" align="center">
                <tr>
                    <td class="label">商品名称：</td>
                    <input type="hidden" name="Goods[id]" value="<?=$dateGoods->id?>"size="30"  />
                    <input type="hidden" name="_csrf-backend" value="<?= Yii::$app->request->csrfToken?>"/>
                    <td><input type="text" name="Goods[name]" value="<?=$dateGoods->name?>"size="30"  />
                    <span class="require-field">*</span></td>
                </tr>
                <tr>
                    <td class="label">商品货号： </td>
                    <td>
                        <input type="text" name="Goods[goods_on]"  size="20"/>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="hidden" name="Goods[category_id]" id="pid" value=""/>
                ...
                <tr>
                    ...
                    <td>
                        <select name="Goods[brand_id]">
                            ...
                            <option value="<?= $v['id']?>" <?= $v['id']==$dateGoods->brand_id?'selected':""?>><?= $v['name']?></option>
                            <?php endforeach;?>
                        </select>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="text" name="Goods[price]" value="<?=$dateGoods->price?>" size="20"/>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="Goods[goods_num]" size="8" value="<?=$dateGoods->goods_num?>"/>
                    </td>
                </tr>
                <tr>
                    <td class="label">是否上架：</td>
                    <td>
                        <input type="radio" name="Goods[status]" value="1" <?=$dateGoods->status?'checked':""?> /> 是
                        <input type="radio" name="Goods[status]" value="0"<?=$dateGoods->status?'':"checked"?>/> 否
                    </td>
                </tr>
                <tr>
                    <td class="label">加入推荐：</td>
                    <td>
                        <input type="checkbox" name="Goods[is_best]" value="1" <?=$dateGoods->is_bast?'checked':""?>/> 精品
                        <input type="checkbox" name="Goods[is_new]" value="1" <?=$dateGoods->is_new?'checked':""?>/> 新品
                        <input type="checkbox" name="Goods[is_hot]" value="1"<?=$dateGoods->is_hot?'checked':""?> /> 热销
                    </td>
                </tr>
                <tr>
                    <td class="label">推荐排序：</td>
                    <td>
                        <input type="text" name="Goods[sort_num]" size="5" value="<?=$dateGoods->sort_num?>"/>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="Goods[keywords]" value="<?=$dateGoods->keywords?>" size="40" /> 用空格分隔
                    </td>
                </tr>
                <tr>
...
<!--                        <input type="file" name="goods_img" size="35" />-->
                        <div id="uploader-demo">
                            ...
                            <div id="img">
                                <?php foreach ($goodsimages as $v):?>
                                    <img src="<?= $v->url?>" class="delete"  width="80"  alt=""/>
                                <?php endforeach;?>
                            </div>
                ...
                <tr>
                    ...
                    <td>
                        <textarea  id="editor"  name="GoodsDetail[detail]" cols="40" rows="3" style="width:650px;height:400px;">
                           <?=$goodsdetail->detail?>
                        </textarea>
<!--                        <script   type="text/plain" style="width:650px;height:400px;"></script>-->
                    </td>
                </tr>
            </table>
...
$(function () {
    $(".delete").dblclick(function(){
        ...
        $.getJSON('<?= \yii\helpers\Url::to(['base/deleteimg'])?>',{"src":src},function (date) {
            if (date.status) {
                that.remove();
    ...
    var zNodes =<?= $goods?>
        ...
        $(document).ready(function(){
            var  treeboj=$.fn.zTree.init($("#treeDemo"), setting, zNodes);
            var  noboj=treeboj.getNodeByParam('id','<?= $model['id']?>');
            $('#pid').val(noboj.id);
            $('#pid_show').val(noboj.name);
            treeboj.selectNode(noboj);
        });
    // 初始化Web Uploader
    ...
    uploader.on( 'uploadSuccess', function(file,date) {
         if(date.status){
             ...
             var img="<img src='"+date.url+"' class='deleteImg' width='80' />";
             $('#img').append(img);
